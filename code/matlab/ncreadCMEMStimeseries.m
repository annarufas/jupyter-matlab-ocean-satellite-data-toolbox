function cmemsData = ncreadCMEMStimeseries(cmemsDatasetsMetadata,...
    fileNameCmemsDataProcessed)

% NCREADCMEMSTIMESERIES Read CMEMS .nc files, extract data and save the
% data into a Matlab structured array for further processing.
%
%   INPUT:
%       cmemsDatasetsMetadata      - string that contains the name of the different CMEMS datasets
%       fileNameCmemsDataProcessed - name of the .mat file containing cmemsData (ensures persistence of data for later use)
%
%   OUTPUT:
...
fprintf("\nReading CMEMS time-series products...")
...
for iDataset = 1:length(cmemsDatasetsMetadata)

    fileName = cmemsDatasetsMetadata{iDataset};
    filePath = fullfile('.','data','raw','CMEMS_data','data_timeseries_areastudy_CMT_nc',fileName);
    ...
    cmemsData(iDataset).ID = erase(fileName, '.nc');
